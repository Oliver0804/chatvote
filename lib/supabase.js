...
async function getHistoryPolls() {
    try {
        ...
        return (data || []).map(poll => ({
            ...poll,
            total_votes: Object.values(poll.votes || {}).reduce((sum, count) => sum + (parseInt(count) || 0), 0)
        }));
    } catch (err) {
...
async function cleanupExpiredPolls() {
    ...
    try {
        const { error } = await supabaseAdmin.rpc('cleanup_expired_chatvote_polls');
